...
class BingoGame:
    ...
    def validate_board(self, board: BingoBoard) -> Tuple[BingoBoard, bool]:
        """
...
def main():
    ...
    game.generate_winning_numbers(count=20)  # Example: draw 20 numbers
    print(f"\nWinning numbers drawn: {game.winning_numbers}")

    # # Validate the board against the drawn winning numbers
    is_winner = game.validate_board(board)
    print(f"\nIs the board a winner? {is_winner}")

    # Example of a guaranteed win scenario (for demonstration/testing)
    lg.info("\n--- Demonstrating a guaranteed win ---")
    ...
    # To guarantee a win, let's make sure the first row is in winning numbers
    first_row_numbers = [
    ...
    guaranteed_win_game.winning_numbers = [1, 4, 2, 3]
    print(f"\nGuaranteed winning numbers: {guaranteed_win_game.winning_numbers}")
    is_guaranteed_winner = guaranteed_win_game.validate_board(guaranteed_board)
    print(f"\nIs the guaranteed board a winner? {is_guaranteed_winner}")
